...
test('desktop overview, source trail, and decision spine are visible', async ({ page }) => {
  ...
  expect(counts.font).toContain('Inter Variable');
  expect(counts.interLoaded).toBe(true);
  ...
  await expect(page.getByRole('tab', { name: 'Max' })).toBeVisible();
...
test('custom dates apply transactionally and reset to Max', async ({ page }) => {
  await page.goto('/?view=timeline&range=10Y');
  await waitForChart(page);
  ...
  await page.locator('[data-custom-range-trigger]').click();
  await page.getByRole('button', { name: 'Reset to Max' }).click();
...
test('theme toggle updates background, buttons, and search panel in lockstep with smooth soulful transition', async ({ page }) => {
  await page.goto('/?view=timeline&range=10Y');
  await waitForChart(page);

  const themeButton = page.locator('.theme-toggle').first();
  const searchButton = page.locator('.header-control').first();
  await expect(themeButton).toBeVisible();
...
test('root landing defaults to Max range and displays all data', async ({ page }) => {
  await page.goto('/');
  await waitForChart(page);
  await expect(page.getByRole('tab', { name: 'Max' })).toHaveAttribute('data-state', 'active');
  await expect(page).toHaveURL(/range=ALL/);
});

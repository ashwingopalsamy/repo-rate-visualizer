...
/** Mobile card row for a single decision */
function DecisionCard({ decision, source, isActive, onSelect }) {
  const trend = getTrend(decision.action);
  return (
    <button
      type="button"
      className={`decision-card w-full text-left transition-all duration-150 rounded-xl border px-4 py-3.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
        isActive
          ? 'decision-card--active border-foreground/30 bg-accent/60 shadow-2xs'
    ...
    >
      {/* Row 1: Date + Action badge + Source link */}
      <div className="flex items-center justify-between gap-2 mb-2">
        <div className="flex items-center gap-2 min-w-0">
          <time className="text-xs font-semibold text-muted-foreground tabular-nums whitespace-nowrap" dateTime={decision.date}>
            {formatDateShort(decision.date)}
          </time>
          <Badge className="px-2 py-0.5 shrink-0" variant={trend.badgeVariant}>
        ...
        {source ? (
          <a
            ...
            title="Open source"
            className="shrink-0 flex size-7 items-center justify-center rounded-lg hover:bg-muted/60 text-muted-foreground hover:text-foreground transition-colors"

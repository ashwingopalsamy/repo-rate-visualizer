...
export default function RateChangeBar({ dateRange }) {
  ...
  const summary = useMemo(() => {
    ...
    const hikes = data.filter(d => d.changeBps > 0);
    const maxCut = cuts.length ? Math.max(...cuts.map(d => Math.abs(d.changeBps))) : 0;
    const maxHike = hikes.length ? Math.max(...hikes.map(d => d.changeBps)) : 0;
        ...
        <div className="flex flex-col gap-3">
          <div className="flex flex-col gap-1">
            <span className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
              Basis Point Moves · Active Adjustments
            </span>
            <h2 className="m-0 text-2xl font-extrabold tracking-tight text-foreground sm:text-3xl">
              {summary.totalMoves} rate moves.
            </h2>
            <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground sm:text-sm">
              The MPC and RBI enacted <strong className="font-semibold text-cut">{summary.cutsCount} cuts</strong> (-{summary.totalCutBps} bps) and <strong className="font-semibold text-hike">{summary.hikesCount} hikes</strong> (+{summary.totalHikeBps} bps), yielding a net change of <strong className="font-semibold text-foreground">{summary.netBps > 0 ? `+${summary.netBps}` : summary.netBps} bps</strong> over the selected window.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 divide-y divide-border/60 rounded-xl border border-border/60 bg-muted/20 shadow-2xs sm:grid-cols-4 sm:divide-y-0 sm:divide-x">
            <div className="p-3 sm:p-3.5">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Active moves</span>
              <div className="mt-1 text-sm sm:text-base font-semibold tabular-nums text-foreground">
                {summary.totalMoves} <span className="text-xs font-normal text-muted-foreground">actions</span>
              </div>
            </div>

            <div className="p-3 sm:p-3.5">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Largest hike</span>
            ...
            <div className="p-3 sm:p-3.5">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Largest cut</span>
            ...
            <div className="p-3 sm:p-3.5">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Net volume</span>
              <div className="mt-1 text-sm sm:text-base font-semibold tabular-nums text-foreground">
                {summary.netBps > 0 ? `+${summary.netBps}` : summary.netBps} bps
              </div>
            </div>
          </div>
        </div>
